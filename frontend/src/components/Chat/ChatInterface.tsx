...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  return (
    <div className="flex flex-col h-full w-full bg-transparent overflow-hidden font-avenir items-center">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto px-4 py-10 space-y-4 scrollbar-none scroll-smooth flex flex-col w-full max-w-[800px]"
        ...
        <AnimatePresence mode='popLayout' initial={false}>
          {messages.map((msg, i) => (
            <motion.div
              ...
              className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} max-w-[80%] ${msg.role === 'user' ? 'self-end' : 'self-start'}`}
            >
              <div className="relative group">
                <div
                  className={`px-5 py-3 rounded-[24px] text-[16px] font-medium leading-relaxed backdrop-blur-xl shadow-2xl border ${
                    msg.role === 'user'
                      ? 'bg-snapchat-blue/40 border-snapchat-blue/30 text-white rounded-br-[4px]'
      ...
      <div className="p-6 pb-10 w-full max-w-[800px]">
